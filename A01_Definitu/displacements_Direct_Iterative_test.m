%% Displacements Direct or Iterative test
...
% 2.4 Solve system using Direct solver
solvertype_D = "Direct";
[u_Direct] = solveSystem(data,K,f,up,vp,solvertype_D);

% 2.4 Solve system using Iterative solver
solvertype_D = "Iterative";
[u_Iterative] = solveSystem(data,K,f,up,vp,solvertype_D);


%% 3) Test

% Load the saved u matrix
load("datas.mat","u");
...
% solver are the same.
assert(all(abs(u_Direct - u_Iterative) < tol , 'all'), 'The displacements using whether a Direct or an Iterative solver are not the same');
