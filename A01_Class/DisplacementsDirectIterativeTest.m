...
% solver are the same.
assert(all(abs(newMainObjectDirect.outputs.u - newMainObjectIterative.outputs.u) < tol , 'all'), 'The displacements using whether a Direct or an Iterative solver are not the same');

% Test to check that the displacements have the expected value using a Direct and an Iterative solver.
assert(all(abs(u - newMainObjectDirect.outputs.u) < tol, 'all'), 'The displacements do not have the expected value using a Direct and an Iterative solver');
